 ...
 */
module.exports = function rankConditions(o, data, limitCategory) {
    ...
    if ((!limitCategory || limitCategory === 'rain') && data.pop > 0.05 && data.rain && !condCheck.rain) {
        const code = [
        ...
        ].filter((level) => { return data.rain > level.min && data.rain <= level.max })[0].code
    ...
    } else if ((!limitCategory || limitCategory === 'rain') && !condCheck.rain && data.rain && data.rain['1h']) {
        // This is only for "current" conditions where somehow "rain" isn't in the "weather" block
        const code = [
            { min: 0, max: 1, code: 311 },
            { min: 1, max: 3, code: 500 },
            { min: 3, max: 10, code: 501 },
            { min: 10, max: 15, code: 503 },
            { min: 15, max: 99, code: 504 }
